 ...
 * 
 */
public abstract class AbstractOuterFetch implements OuterFetch {
	...
	public int getImageCodeUrl(String wwwUrl) throws Exception {
		...
		try {
			...
			if (S_Logger.isDebugEnabled())
				S_Logger.debug("transit_nb - AbstractOuterFetch" + e.getMessage());
		} finally {
			if (br != null) {
				br.close();
			}
	...
	public String getValidateCode(String imageUrl) throws Exception {
		...
		URL url = null;
		HttpURLConnection conn = null;
		S_Logger.debug("------in getValidateCode--------");
		...
		S_Logger.debug("------conn disconnect--------");
		// 解析验证码
		S_Logger.debug("-------" + imageFile.getPath() + "-------");
...
//		return httpTest.getResult(httpTest.upload(imageFile.getPath(), "1004", false));
	...
	public String login(String loginUrl , String formData) throws Exception {
		...
		Reader r = new InputStreamReader(buff, encoding);
